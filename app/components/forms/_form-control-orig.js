import BaseComponent from 'app/components/application-component';
...
export default BaseComponent.extend(FormValidationOnChange, {
    /**
    ...
    afterRender: function () {
        ...
        // Check for show-hide setting
        this.checkShowHideSetting();
    ...
    /**
     * Computed property that determines the CMS element ID
     *
     * @return {string} displayElementID
     */
    displayElementID: function() {
        var displayElementID;

        if (this.get('environment') === 'local' || this.get('environment') === 'dev') {
            if (this.get('controllerModelMeta.identifier')) {
                displayElementID = this.get('controllerModelMeta.identifier');
            } else {
                displayElementID = 'ID not found';
            }
        } else {
    ...
    lookForParentTable: function () {
        ...
        if (this.$().closest('table').length) {
            // If in admin show/hide mode make sure that form fields in table cells have a label.
            // In regular mode, multiple form fields in a table cell would have placeholder values,
            // however placeholder have to be removed in admin show/hide mode for the selector mask.
            if (this.showHideAdmin.get('isEditMode') && !this.get('label')) {
                this.set('label', this.get('placeholder'));
            }
            // Set a flag on the object
            this.set('insideTable', true);
            // Turn off component info
            if (!this.$().closest('.repeater-list').length && !this.$().closest('.TODO').length) {
                this.set('componentInfo', false);
            }
